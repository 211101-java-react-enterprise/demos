...
let arr1: number[] | boolean[]; //we've declared a number array 
arr1 = [1,2,3,4];

let arr2: Array<number|boolean>; //pretty much an indentical way of declaring a number array 
arr2 = [1,true];
...
//Below is the TS way 
enum carStates {OFF, IDLE, STOPLIGHT, ACCEL, CRUISING}; //TS will also automatically number the values!
...
function finalFunc(a: number|boolean,b: Array<boolean|string>, c: unknown): string|boolean|void{
    //any vs unknown:
...
interface Planet{
    ...
    hasRings: boolean,
    moons: Array<Moon>,
    orbit(fake: number): void; //declaring that any Planet types must have an orbit function
}
...
class Star implements Moon{
    ...
    /**
     *  A class can implement an interface and the interface forces it to declare the properties that the interface defines. 
     */
...
class Animal {
    ...
    /**
     * implicit modifier in Java if no access modifier is declared? package access level. 
     * 
     * in TS the default access modifer is public!
     */
    name: string; //implicilty the name is public 
    public neutered: boolean; //this is accesible anywhere!
    protected breed: string; //accessible internally to the class or to any class that extends it. 
    private age: number; //accessiable only to our class. 

    constructor(name: string, neutered: boolean, breed: string, age: number){
        this.age = age;
        this.name = name;
